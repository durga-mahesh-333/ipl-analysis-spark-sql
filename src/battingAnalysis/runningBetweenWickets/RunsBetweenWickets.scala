...
object RunsBetweenWickets {
  def main(args: Array[String]): Unit = {
    ...
    val strikeRateDf = sqlContext.sql("select striker , SUM( CASE WHEN ( runs_in_that_deleivery!=4 AND runs_in_that_deleivery !=6) THEN runs_in_that_deleivery ELSE 0 END) as runs_witout_boundaries ,SUM(CASE WHEN ( (extras == 0 OR runs_in_that_deleivery!=0) AND ( runs_in_that_deleivery!=4 AND runs_in_that_deleivery !=6)) THEN 1 ELSE 0 END) as balls  from IPLDataTable  where innings <3 AND season>=2015   Group by striker  having SUM(runs_in_that_deleivery)>300 ");
    var resultDF = strikeRateDf.select(col("striker"), bround(col("runs_witout_boundaries").divide(col("balls")), 3).alias("value")).sort(col("value").desc);
